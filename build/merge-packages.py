...
No single .NET SDK can build net8, net9 and net10 for iOS: each SDK's workload supports the
current target framework and the previous one only. Verified for this repository:

                SDK 9 band                          SDK 10 band
    iOS         net8.0-ios18.0,     net9.0-ios18.0  net10.0-ios26.0

The package is therefore built in two passes (see BuildNugets.sh) and merged here into one.
Generic otherwise — carried over unmodified from the sibling Net.Agora.iOS repository, which
packs several package ids per run rather than one.
